...
data.downloads <- ddply(dateRanges, .(number), 
                        function(x) {getDownloads(projectId, 
                                                  x$beginTimestamp, 
                                                  x$endTimestamp)})

foo <- data.downloads %>% filter(RESPONSE_STATUS == 200)  %>% count(userid, id, DATE, TIMESTAMP, NODE_TYPE) %>% ungroup() %>% 
...
data.downloads <- left_join(foo, allEntities, by="id")

```

### Download usage statistics from `r min(dateRanges$begin)` to `r max(dateRanges$end)`
